...
Android 渲染体系设计中，硬件渲染是主流方案，但软件渲染在某些轻量级场景下仍有其用武之地。然而，软件渲染隐藏着一个容易被忽视的问题——帧同步（Frame Synchronization）导致的丢帧现象。本文将从实际问题和原理出发，深入分析软件渲染为何会导致丢帧，以及 VSYNC、fence 在其中扮演的角色。
...
最近在做一个轻量级渲染场景，采用了软件渲染（Software Rendering）方案。测试过程中发现了一个奇怪的现象：画面会出现轻微的抖动。排查后发现，这种抖动并非性能不足导致，而是帧同步（Frame Synchronization）出了问题。

具体来说：软件渲染的帧提交后，有时能顺利上屏，有时却会出现丢帧。更为诡异的是，这种丢帧并非发生在渲染慢的场景，反而发生在渲染快的场景。这篇文章就带你深入理解这背后的原理。
...
软件渲染的特点是 CPU 直接完成绘制，不需要 GPU 参与。这意味着渲染速度极快，可能在 **不到一个 VSYNC 周期**内就完成了。

想象这样的时序：

```
T0: VSYNC-App 到达，应用开始渲染
T1: 软件渲染瞬间完成（非常快）
T2: 提交到 SurfaceFlinger
T3: SurfaceFlinger 开始合成
```
